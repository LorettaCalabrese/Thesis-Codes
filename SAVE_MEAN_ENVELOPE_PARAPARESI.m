...
name_file_excel="C:\Users\Utente\Desktop\TESI MAGISTRALE\DATI EMG\PATOLOGICI\PARAPARESI\EXCEL_PARAPARESI\NUM_SAMPLES_PARAPARESI.xlsx";
...
if strcmp(Patology,"PARAPARESI")

 values_WALK=repelem(1:length(A),(floor(mean_n_samples)));
 results_WALK=strcat(string(values_WALK(:)),results_Velocity);

elseif ~strcmp(Patology, "PARAPARESI" ) && isempty(Velocity) %when patology is ATASSIA or PARAPARESI, Velocity is a blank space
 values_WALK=repelem(1:length(A),(floor(mean_n_samples)));
 results_WALK=values_WALK(:);
